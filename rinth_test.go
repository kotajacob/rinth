...
func testJSONMarshal(t *testing.T, v interface{}, want string) {
	...
	if err := json.Unmarshal([]byte(want), &u); err != nil {
		t.Errorf("rinth unable to unmarshal JSON for %v: %v", want, err)
	}
	w, err := json.Marshal(u)
	if err != nil {
		t.Errorf("rinth unable to marshal JSON for %#v", u)
	}

	// Marshal the target value.
	j, err := json.Marshal(v)
	if err != nil {
		t.Errorf("rinth unable to marshal JSON for %#v", v)
	}

	if diff := cmp.Diff(string(w), string(j)); diff != "" {
		t.Errorf("rinth json.Marshal(%q):\n%s", v, diff)
	}
}

func testMethod(t *testing.T, r *http.Request, want string) {
	if got := r.Method; got != want {
		t.Errorf("rinth request method: %v, want %v", got, want)
	}
}
